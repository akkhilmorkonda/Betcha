 ...
 *
 *   npm run db:snapshot   copy dave.db -> dave.demo.db
...
const LIVE = resolve("prisma/dave.db");
const SNAP = resolve("prisma/dave.demo.db");
...
if (mode === "save") {
  if (!existsSync(LIVE)) {
    console.error("\n  No prisma/dave.db yet. Run npm run db:reset first.\n");
    process.exit(1);
  }
  copyFileSync(LIVE, SNAP);
  console.log(`\n  Snapshot saved — prisma/dave.demo.db (${kb(SNAP)})`);
